...
internal sealed class MenuViewModel
{
    ...
    public MenuViewModel(Application application, IMenu menu, MenuViewModel? duplicate)
    {
        Order = menu.Order;
        Header = menu.Path[^1];
        Icon = menu.Icon;
    ...
    public static int Compare(MenuViewModel menu1, MenuViewModel menu2)
    {
        ...
        comparison = comparison != 0 ? comparison : (menu1.SubMenus?.Count ?? 0).CompareTo(menu2.SubMenus?.Count ?? 0);

        return comparison;
    }

    public bool Remove(MenuViewModel subMenu)
    {
        return _subMenus?.Remove(subMenu) ?? false;
    }
